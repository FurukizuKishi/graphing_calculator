...
class Window(QtGui.QMainWindow):
            ...
            for x in range(len(Layout[y])):
                ...
                if Layout[y][x] != " ":
                    ...
                    #Create lg, ln and log buttons.
                        ...
                        self.Button(cx, y*gbh, min(self.winW, bw, self.winW - cx), gbh, "lg", self.CalculatorWidgets, True, "default", self.THREADUpdate, ("G(", False, 0, 0, min(self.winW, round((len(Layout[0][0]) - 1) / 3) * gbw), gbh))
                    elif Layout[y][x] == "Ln":
    ...
    def Calculate(self, GiveAnswer, x, y, w, h):
        ...
        print(self.CalculatorWidgets[22][0])
        if self.Equation != "":
        ...
        #Throw up an error if a logarithm has no base.
        if "L(" in self.Equation:
            self.Answer = "Math Domain Error"
        ...
        if not isinstance(self.Answer, str):
            ...
            if self.CalculatorWidgets[40][0].text() == "0":
                ...
                self.Answer = int(round(self.Answer, int(self.CalculatorWidgets[41][0].text())))
            #Otherwise...
            else:
                #Round it, making it a float.
                self.Answer = round(self.Answer, int(self.CalculatorWidgets[41][0].text()))
    ...
    def FormatExpression(self, Equation, CalculatorExpression):
        ...
        Operators = "+-×÷^"
        Functions = [["S(", "sin("], ["C(", "cos("], ["T(", "tan("], ["E(", "ln("], ["G(", "lg("], ["A(", "abs("]]
        ...
        e = '<sub>' + '\N{REPLACEMENT CHARACTER}' + '</sub>'
        CalculatorExpression = CalculatorExpression.replace("L(", "log"+e+"(")
        if (Term != "L(") or ("L" not in Term):
            Encapsulations = [["S", "C", "T", "L", "E", "G", "A", "("], [")"]]
            while CalculatorExpression.count("L") > 0:
                index = CalculatorExpression.index("L")
                print("Index:",index)
                e = ''
                print("Length:",len(CalculatorExpression))
                #Move through the characters of the power.
                for i in range(index+1, len(CalculatorExpression)):
                    #If there are no brackets or functions, only power a single term.
                    print("CE:",CalculatorExpression[i])
                    if CalculatorExpression[index+1] not in Encapsulations[0]:
                        if CalculatorExpression[i] in ("1234567890." + Variables):
                            e = e + CalculatorExpression[i]
                            print('e:',e)
                ...
                print("")
                eo = e
                #If there are no characters of the logarithm base, generate an empty slot to allow the user to know they are inputting one.
                if e == '':
                    e = '<sub>' + '\N{REPLACEMENT CHARACTER}' + '</sub>'
                else:
                    e = '<sub>' + e + '</sub>'
                print("eo:",eo)
                print("New e:",e)
                CalculatorExpression = CalculatorExpression.replace("L"+eo, "log"+e)
    ...
    def UpdateExpression(self, char, GiveAnswer, x, y, w, h):
        ...
        Variables = "πφeabcdefghijklmnopqrstuvwxyz"
        Operators = "+-×÷^"
        Functions = [["S(", "sin("], ["C(", "cos("], ["T(", "tan("], ["E(", "ln("], ["G(", "lg("], ["A(", "abs("], ["L(", "log("]]
        Primary = Digits + Variables + "-("
        Precendentary = ")!"
        Prohibited = ["√x", "^", "+", "-", "×", "÷"]
        print(Digits[:-1])
    ...
    def expressionTextChanged(self):
        if self.CreatingGraph == True:
            if self.PropertiesWidgets[0][0].text() == '':
                ...
                functionText = [["sin", "S"], ["cos", "C"], ["tan", "T"], ["log", "L"], ["ln", "E"], ["lg", "G"], ["abs", "A"]]
                CurveText = self.PropertiesWidgets[0][0].text()
                for i in range(0, len(functionText)):
                    CurveText = CurveText.replace(functionText[i][0], functionText[i][1])
                print(CurveText)
