...
def Differentiation(e, var):
    for i in range(0, len(e[0])):
        if isinstance(e[0][i], int) or isinstance(e[0][i], float):
            e[0][i] = str(e[0][i])
    #print(e)
    e = ''.join(e[0])
    #print(e)
    ...
    for i in range(0, len(var)):
        ...
        if e[0] == var[i][0]:
            e = "1*" + e
            #print("u:", e)
        for ii in range(0, len(Operators)):
            e = e.replace(var[i][0] + Operators[ii], var[i][0] + "^1" + Operators[ii])
            e = e.replace(Operators[ii] + var[i][0], Operators[ii] + "1*" + var[i][0])
    try:
        #print("e0:", e)
        ComplexDiff = [
                        ["S(k*x)", "k*C(k*x)"],
        ...
        for f in range(0, len(Functions)):
            ...
            while e[i].count(Functions[f][0]) > 0:
                ...
                for p in range(1, len(Functions[f])):
                    #print("START: " + str(e[i][ii+1]), end = "\n")
                    if Functions[f][0] == "T":
                        if (e[i][ii+1] - 90) % 90 == 0:
                            return [["ERROR", "Complex Error"]]
                    if Functions[f][0] == "L":
                        try:
                            #print("log" + str(e[i][ii+1]) + "(" + str(e[i][ii+2]) + ")")
                            e[i][ii+2] = math.log(e[i][ii+2], e[i][ii+1])
                            del e[i][ii+1]
                            break;
                        except:
                            pass;
        ...
        while y < len(e_temp[x]):
            ...
            if y > 0:
                ...
                if e_temp[x][y] not in operators:
                    if y < len(e_temp[x]) - 1:
                        digits.join('')
                        if e_temp[x][y+1] in digits and e_temp[x][y] in digits:
...
#Input("546-(123*2", "Calculation", [['x', 3], ['y', 1]])
#Input("2*x", "Differentiation", [['x']])
#Input("L10(5)", "Calculation", [])
